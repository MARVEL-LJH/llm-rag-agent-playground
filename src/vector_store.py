...
# 只保留新版，删除 langchain_community 的旧Embedding导入
...
# 全局唯一embedding实例，全部函数统一复用这一个，不要重复new
embeddings = HuggingFaceEmbeddings(
    model_name=LOCAL_EMBED_MODEL_PATH,   # 指向本地文件夹！不是在线名字
    model_kwargs={"device": "cpu"},
    encode_kwargs={"normalize_embeddings": True},
    local_files_only=True,   # ⭐最重要！禁止访问huggingface网络，只读取本地磁盘文件
)


def load_vector_db():
    """加载已经保存好的向量库，给rag_qa调用"""
    # ❗修复：复用全局embeddings，不再重新实例化，不再写在线模型名
    db = Chroma(
...
def main():
    ...
    # ❗修复：复用全局embeddings，不要再次创建HuggingFaceEmbeddings
